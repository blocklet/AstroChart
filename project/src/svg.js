...
(function (astrology) {
  ...
  astrology.SVG.prototype._getSymbol = function (name, x, y) {
    switch (name) {
      ...
      case astrology.SYMBOL_AS:
        ...
        break;
      case astrology.SYMBOL_ASCENDANT:
        return ascendantForPoints(x, y);
        break;
  ...
  function getSignWrapperId(sign) {
    return (
      astrology._paperElementId + "-" + astrology.ID_RADIX + "-" + astrology.ID_SIGNS + "-" + sign
    );
  }
  ...
  function getHouseIdWrapper(house) {
    return (
      astrology._paperElementId + "-" + astrology.ID_RADIX + "-" + astrology.ID_CUSPS + "-" + house
    );
  }
  ...
  /**
   * Draw As points symbol
   */
  function ascendantForPoints(x, y) {
    // center symbol
    var xShift = 12; //px
    var yShift = -2; //px
    x = Math.round(x + xShift * astrology.SYMBOL_SCALE);
    ...
    node.setAttribute("stroke", astrology.POINTS_COLOR);
    node.setAttribute("stroke-width", astrology.POINTS_STROKE * astrology.SYMBOL_SCALE);
    node.setAttribute("fill", "none");
