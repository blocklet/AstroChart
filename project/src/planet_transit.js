...
(function (astrology) {
   ...
   * @param {Object} data
   */
  astrology.PlanetTransit = function (radix, data) {
    // Validate data
   ...
   * Draw background
   */
  astrology.PlanetTransit.prototype.drawBg = function () {
   ...
   */
  astrology.PlanetTransit.prototype.drawPoints = function (planetsData) {
    ...
    var wrapper = astrology.utils.getEmptyWrapper(
      universe,
      astrology._paperElementId + "-" + astrology.ID_TRANSIT + "-" + astrology.ID_POINTS,
    );
    ...
    var gap =
      ...
      (this.radius / astrology.INNER_CIRCLE_RADIUS_RATIO +
        this.radius / astrology.INDOOR_CIRCLE_RADIUS_RATIO);
    var step =
      (gap - 2 * (astrology.PADDING * astrology.SYMBOL_SCALE)) / Object.keys(planets).length;
    ...
    for (var planet in planets) {
      if (planets.hasOwnProperty(planet)) {
        var position = astrology.utils.getPointPosition(
          this.cx,
          this.cy,
          this.pointRadius + astrology.PADDING * 1.5,
    ...
    if (astrology.DEBUG)
      console.log("Transit located points:\n" + JSON.stringify(this.locatedPoints));
    ...
    this.locatedPoints.forEach(function (point) {
      ...
      if (!astrology.STROKE_ONLY && planets[point.name][0] + this.shift != point.angle) {
        ...
        endPosition = astrology.utils.getPointPosition(
          this.cx,
          this.cy,
          this.pointRadius - astrology.COLLISION_RADIUS * astrology.SYMBOL_SCALE,
          point.angle,
        );
      ...
      // draw symbol
      var symbol = this.paper.getSymbol(point.name, point.x, point.y, true);
      symbol.setAttribute(
        "id",
   ...
   * Draw circles
   */
  astrology.PlanetTransit.prototype.drawCircles = function drawCircles() {
    var universe = this.universe;
    var wrapper = astrology.utils.getEmptyWrapper(
      universe,
      astrology._paperElementId + "-" + astrology.ID_TRANSIT + "-" + astrology.ID_CIRCLES,
    );
   ...
   */
  astrology.PlanetTransit.prototype.drawCusps = function (cuspsData) {
    ...
    var wrapper = astrology.utils.getEmptyWrapper(
      universe,
      astrology._paperElementId + "-" + astrology.ID_TRANSIT + "-" + astrology.ID_CUSPS,
    );
    var numbersRadius =
      this.radius + (this.radius / astrology.INNER_CIRCLE_RADIUS_RATIO - this.rulerRadius) / 2;
    ...
    for (var i = 0, ln = cusps.length; i < ln; i++) {
      ...
      var gap =
        endOfCusp - startOfCusp > 0 ? endOfCusp - startOfCusp : endOfCusp - startOfCusp + deg360;
  ...
  astrology.PlanetTransit.prototype.drawRuler = function drawRuler() {
    var universe = this.universe;
    var wrapper = astrology.utils.getEmptyWrapper(
      universe,
      astrology._paperElementId + "-" + astrology.ID_TRANSIT + "-" + astrology.ID_RULER,
    );
   ...
   */
  astrology.PlanetTransit.prototype.aspects = function (customAspects) {
    var aspectsList =
    ...
    for (var i = 0, ln = aspectsList.length; i < ln; i++) {
      ...
      line.setAttribute(
        "stroke",
        astrology.STROKE_ONLY ? astrology.LINE_COLOR : aspectsList[i].aspect.color,
      );
      line.setAttribute("stroke-width", astrology.CUSPS_STROKE * astrology.SYMBOL_SCALE);
  ...
  astrology.PlanetTransit.prototype.transit = function(from, to, color){
    const min = Math.min(from, to)
    const max = Math.max(from, to)
    const segment = this.paper.segment(this.cx, this.cy, this.radius + this.radius / astrology.INNER_CIRCLE_RADIUS_RATIO, min, max, 0, 0, 0)
    const wrapper = astrology.utils.getEmptyWrapper(this.universe, this.paper.root.id + '-' + astrology.ID_ASPECTS, this.paper._paperElementId)
    segment.setAttribute('data-name', 'segment-transit')
    segment.setAttribute('fill', color)
    wrapper.appendChild(segment)
    const text = this.paper.text((max-min).toFixed(1).toString() + '°', this.cx, this.cy,  '14sp',  '#EEE')
   ...
   */
  astrology.PlanetTransit.prototype.animate = function (data, duration, isReverse, callback) {
    // Validate data
    var status = astrology.utils.validate(data);
    if (status.hasError) {
      throw new Error(status.messages);
